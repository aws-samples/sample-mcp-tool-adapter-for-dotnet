...
`net10.0` assets only, so it cannot run here at all, and retargeting a WebForms application to .NET 8 is
the rewrite the whole exercise is meant to avoid.
...
`ToolRegistry`, and the same `DataTable` and `DataSet` return types, with nothing but a different host
...
`Application_Start`. `McpToolAdapter.Web` carries a `PreApplicationStartMethod` attribute, so it registers
its own module before any application code runs. Look at [`Web.config`](Web.config) and note what is
absent.
...
so a change that breaks .NET Framework compatibility fails the build immediately. It has not been executed
under IIS. The `System.Web` host it exercises carries the same caveat, recorded in the root
[README](../../README.md).

Building it surfaced one real incompatibility, which is the point of having it. `Services.cs` used
`Math.Clamp`, which arrived in .NET Core 2.0 and does not exist on .NET Framework. It now uses `Math.Min`
and `Math.Max`. A shared file compiling for both targets is what caught that, and neither sample alone
would have.

## Requirements

- .NET Framework 4.7.2 or later.
- The IIS integrated pipeline. Under the classic pipeline an extensionless path such as `/_mcp/health`
  never reaches managed code, so the self-registering module never sees it. Use `McpHandler` with an
  explicit `.ashx` there instead; see its remarks in `McpToolAdapter.Web`.
